...
# summary statistics ----
summarytools::view(stby(meta.data$Age, meta.data$Diagnosis, descr,
  transpose = T,
  stats = c("mean", "sd")
), file = "Graphs/Age Description by Diagnosis.html", footnote = NA)
summarytools::view(stby(meta.data$Age, meta.data$Sex, descr,
  transpose = T,
  stats = c("mean", "sd")
), file = "Graphs/Age Description by Sex.html", footnote = NA)
summarytools::view(dfSummary(meta.data,
  plain.ascii = F, style = "grid",
  graph.magnif = 0.75, valid.col = F, tmp.img.dir = "/tmp"
), file = "Graphs/Meta Data Summary.html", footnote = NA)
...
lines <- c(
  paste(c("ID", names(lefse.dataset)), collapse = "\t"),
  sapply(seq_len(nrow(lefse.dataset)), function(i) {
    paste(c(
      row.names(lefse.dataset)[i],
      lefse.dataset[i, ]
    ), collapse = "\t")
  })
)
...
# alpha diversity ----
plot <- plot_richness(phy.seq,
  x = "Diagnosis", color = "Sex",
  measures = c("Chao1", "Shannon", "simpson", "ace")
)

plot + geom_boxplot(
  data = plot$data, aes(x = Diagnosis, y = value),
  color = "#53B74C", alpha = 0.1
)

ggsave("Graphs/Alpha Diversity Measures on OTUs Abundances.png",
  width = 2931,
  height = 1782, units = "px"
)

richness <- estimate_richness(phy.seq,
  split = T, measures =
...
rel.eig.val <- phy.ord$values$Relative_eig
x.lab <- sprintf("PC1 (%.1f%%)", rel.eig.val[1] * 100)
y.lab <- sprintf("PC2 (%.1f%%)", rel.eig.val[2] * 100)
...
ggsave("Graphs/Abundance of Phylum-level Microbiota.png",
  width = 2931,
  height = 1782, units = "px"
)
...
abundances.by.bacteria <- abundances %>%
  group_by(Bacteria) %>%
  dplyr::summarize(Abundance = sum(Abundance)) %>%
  as.data.frame()
...
abundances.by.diagnosis <- abundances %>%
  group_by(Diagnosis) %>%
  dplyr::summarize(Abundance = sum(Abundance)) %>%
  as.data.frame()
...
ggsave("Graphs/Abundance of Genus-level Microbiota.png",
  width = 2931,
  height = 1782, units = "px"
)
...
abundances.difference <- data.frame(
  Bacteria = rownames(abundances.table),
